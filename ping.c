...
void listener() {
  ...
  if (setsockopt(sd, SOL_IP, IP_TTL, &val, sizeof(val)) != 0) {
    ERRM(
        "Error: could not set TTL option  - did you forget to run as root or "
        "sticky bit cnping?\n");
    exit(-1);
  }
#endif
  ...
  keep_retry_quick:
    ...
    bytes =
        recvfrom(sd, buf, sizeof(buf), 0, (struct sockaddr*)&addr, &addrlenval);
    if (bytes == -1)
      continue;
    if (buf[20] != 0)
...
void ping(struct sockaddr_in* addr) {
  ...
  do {
    ...
    memset(&pckt.hdr, 0,
           sizeof(pckt.hdr));  // This needs to be here, but I don't know why,
                               // since I think the struct is fully populated.
    ...
    pckt.hdr.checksum =
        checksum((const unsigned char*)&pckt, sizeof(pckt.hdr) + rsize);

    int sr = sendto(sd, (char*)&pckt, sizeof(pckt.hdr) + rsize, 0,
  ...
  // close( sd ); //Hacky, we don't close here because SD doesn't come from
  // here, rather  from ping_setup.  We may want to run this multiple times.
}
...
void ping_setup() {
  ...
  {
    int lttl = 0xff;
    if (setsockopt(sd, IPPROTO_IP, IP_TTL, (const char*)&lttl, sizeof(lttl)) ==
        SOCKET_ERROR) {
  ...
  if (setsockopt(sd, SOL_IP, IP_TTL, &val, sizeof(val)) != 0) {
    ERRM(
        "Error: Failed to set TTL option.  Are you root?  Or can do sock_raw "
        "sockets?\n");
    exit(-1);
  }
